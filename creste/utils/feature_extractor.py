...
def use_sklearn_pca(features: torch.Tensor, n_components: int):
...
def compute_pca_reduction(features, max_feats_to_compute_pca, pca_target_feature_dim):
    selected_features = features[
        torch.randperm(features.shape[0])[:max_feats_to_compute_pca]
    ]
    U, S, reduce_to_target_dim_mat = torch.pca_lowrank(
        selected_features, q=pca_target_feature_dim, niter=20
    )
    ...
    return reduce_to_target_dim_mat

def extract_vit_features(extractor, img_path, input_shape):
    """Expects input_shape to be (H, W)"""

    # If type is string
    if isinstance(img_path, str):
        #1 Load torch transform
        ...
        img_th = prep(img).unsqueeze(0).cuda()
    ...
    B, _, _, F = descriptors.shape
    H, W = extractor.num_patches
    # descriptors =  descriptors.squeeze().view(H*W, -1)
    descriptors = descriptors.view(B, H*W, F)

    return descriptors # [B, Hs*Ws, C]
...
class ViTExtractor(nn.Module):
    ...
    def _fix_pos_enc(patch_size: int, stride_hw: Tuple[int, int]):
        ...
        def interpolate_pos_encoding(
            ...
            w0, h0 = w0 + 0.1, h0 + 0.1
            scale_h = float(w0 / math.sqrt(N))
            scale_w = float(h0 / math.sqrt(N))
